...
# COMPARING MODEL BY PLOTTING THEM

import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np

# Sample Data (Scores in percentage)
models = ['Linear Regression', 'SVR', 'Random Forest', 'Gradient Boosting', 'XGBoost']
train_scores = [93, 5, 84, 86, 98]
test_scores = [91, 8, 82, 87, 98]
cv_scores = [90, 8, 83, 86, 98]

tuned_models = ['Random Forest (Tuned)', 'Gradient Boosting (Tuned)', 'XGBoost (Tuned)']
tuned_scores = [92, 91, 93]

# Initialize the figure with enhanced aesthetics
...
# Untuned Model Performance Comparison
bar_width = 0.25
x_indices = np.arange(len(models))

axes[0].bar(x_indices - bar_width, train_scores, width=bar_width, color="#1f77b4", label='Train Score')
axes[0].bar(x_indices, test_scores, width=bar_width, color="#ff7f0e", label='Test Score')
axes[0].bar(x_indices + bar_width, cv_scores, width=bar_width, color="#2ca02c", label='CV Score')

# Axis labels and ticks
axes[0].set_title("Untuned Model Performance", fontsize=14)
axes[0].set_ylabel("R² Scores (%)", fontsize=12)
axes[0].set_xticks(x_indices)
axes[0].set_xticklabels(models, rotation=10, ha='center', fontsize=10)
axes[0].set_ylim(0, 100)
axes[0].legend(loc="upper left", frameon=False, fontsize=10)

# Tuned Model Performance Comparison
sns.barplot(x=tuned_models, y=np.array(tuned_scores), ax=axes[1], palette="Purples", alpha=0.9)
axes[1].set_title("Tuned Model Performance", fontsize=14)
axes[1].set_ylabel("R² Scores (%)", fontsize=12)
axes[1].set_xticks(np.arange(len(tuned_models)))
axes[1].set_xticklabels(tuned_models, rotation=10, fontsize=10)
